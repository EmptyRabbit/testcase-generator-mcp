...
- 本 prompt 返回后，**你（当前对话的 AI）就是本流程的唯一执行者**。
- **必须**将以下步骤在本轮对话中**按顺序**执行，不得跳过、不得合并需要用户确认的步骤。
- 凡步骤中要求“输出以下内容”或“暂停任务，等待用户输入后继续”的，**必须先完整输出指定内容，然后停止回复，等待用户输入**
  ；未收到用户回复前**严禁**执行下一步或调用其他工具。
- 违反上述任一条即视为未完成流程，不得交付最终产物（.md/.xmind）。
...
- 所有子任务中，凡是步骤说明中提到"输出以下内容"或"输出步骤X的结果"，以及包含"**暂停任务，等待用户输入后继续**"字样的，必须：
    1. 将指定内容**完整输出**到一轮用户对话中（必须在对话中输出，输出在Thinking中的内容无效），确保用户能看到；
    2. **停止执行后续步骤**，等待用户在对话中回复信息；
    3. 收到用户回复后，再继续执行下一步骤。
- **严禁**在未收到用户回复的情况下，自动跳过暂停点、连续执行多个需要用户确认的步骤。

### 1. 环境检查

- 步骤1：当需要生成 .xmind 文件时，检查当前环境是否配置了 MCP： `xmind-generator-mcp`（仅检查工具列表，不要调用）
- 步骤2：如果存在未配置的 MCP，请在用户对话中输出以下内容，**暂停任务，等待用户输入后继续**：
...
- 步骤1：如果用户提供了链接，优先调用工具读取内容，分析上下文，拆解功能架构，规划需要测试的模块
    ...
    - 模块内容需要用反引号``包裹
- 步骤2：严格按以下格式输出步骤1的结果，请在用户对话中输出以下内容，**暂停任务，等待用户输入后继续**：
...
- 步骤3：严格按以下格式，按各个一级模块，请在用户对话中输出步骤1、步骤2的结果，**暂停任务，等待用户输入后继续**：
...
- 步骤2：根据 `用例设计原则` 和 `用例优先级定义` 的内容，结合上下文信息和需求追溯矩阵，设计和生成测试用例，并按照以下格式，写入
  .md 文件
    - Markdown 格式
      ```markdown
      # 需求名
